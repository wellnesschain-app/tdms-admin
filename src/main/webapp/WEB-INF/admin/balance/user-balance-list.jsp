...
<script>
                    ...
                    if(res.msg=="ok"){
                        ...
                        if(res.balance.list.length<=0){
                            ...
                            tr+=' <tr><td><input type="checkbox" value="'+v.id+'" name="">' + ' </td>' +
                                ...
                                '<td>' +v.ustdBalance+'</td>' +
                                '<td>' +statuss+'</td>' +
                                '<td><a style="cursor: pointer" onclick="showContent('+v.id+',\''+v.addr+'\')">转入</a>&nbsp|&nbsp;<a style="cursor: pointer" onclick="showContent1('+v.id+',\''+v.addr+'\')">转出</a></td></tr>'
    ...
    function showContent1(id,addr) {
        layer.open({
            ...
            area:['750px','350px'],
            shadeClose: true, //开启遮罩关闭
            content:"${ctx}/admin/user/toTransferReduce?id="+id+"&addr="+addr

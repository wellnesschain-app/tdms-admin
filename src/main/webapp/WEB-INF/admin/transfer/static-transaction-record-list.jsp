...
<div class="x-body">
    ...
    <xblock>共有数据：${staticBonusRecord.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${staticBonusRecord.list.size()==0}">
            <tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${staticBonusRecord.list.size()>0}">
            <c:forEach items="${staticBonusRecord.list}" var="t">
                <tr>
                    <td>
    ...
    <input type="hidden" id="pages" value="${staticBonusRecord.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            var id = '${id}';
            $.ajax({
                url:"${ctx}/admin/transfer/getStaticBonusPage",
                type:"GET",
                data:{"pageNum":curr,"pageSize":10,"id":id},
                dataType:"json",
                success:function(res){
                    if(res.msg=="ok"){
                        ...
                        var tr = "";
                        var type="";
                        $("#pages").val(res.staticBonusRecord.pages);
                        if(res.staticBonusRecord.list.length<=0){
                            tr='<tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>'
                        }else{
                            $.each(res.staticBonusRecord.list,function (i, v) {
                                if(v.status==5){
                                    statusss="静态收益";
                                }

...
<div class="x-body">
    ...
    <%--</form>--%>
        ...
        <br>
    <xblock>共有数据：${JFTransfer.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${JFTransfer.list.size()==0}">
            <tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${JFTransfer.list.size()>0}">
            <c:forEach items="${JFTransfer.list}" var="t">
                <tr>
                    <td>
    ...
    <input type="hidden" id="pages" value="${JFTransfer.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            $.ajax({
                url:"${ctx}/admin/transfer/getJFTransfer",
                type:"GET",
                ...
                success:function(res){
                    if(res.msg=="ok"){
                        ...
                        var tr = "";
                        var type="";
                        $("#pages").val(res.JFTransfer.pages);
                        if(res.JFTransfer.list.length<=0){
                            tr='<tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>'
                        }else{
                            $.each(res.JFTransfer.list,function (i, v) {
                                if(v.F04==1){
    ...
    function search() {
        ...
        window.location.href="${ctx}/admin/transfer/toJFTransferSearch?start_time="+start_time+"&end_time="+end_time+"&fromid="+fromid+"&toid="+toid;
    }
</script>

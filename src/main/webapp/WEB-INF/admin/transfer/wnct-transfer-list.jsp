...
<div class="x-body">
    ...
    <%--</form>--%>
        ...
        <br>
    <xblock>共有数据：${WNCTTransfer.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${WNCTTransfer.list.size()==0}">
            <tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${WNCTTransfer.list.size()>0}">
            <c:forEach items="${WNCTTransfer.list}" var="t">
                <tr>
                    <td>
    ...
    <input type="hidden" id="pages" value="${WNCTTransfer.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            $.ajax({
                url:"${ctx}/admin/transfer/getWNCTTransfer",
                type:"GET",
                ...
                success:function(res){
                    if(res.msg=="ok"){
                        ...
                        var tr = "";
                        var type="";
                        $("#pages").val(res.WNCTTransfer.pages);
                        if(res.WNCTTransfer.list.length<=0){
                            tr='<tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>'
                        }else{
                            $.each(res.WNCTTransfer.list,function (i, v) {
                                if(v.F04==1){
    ...
    function search() {
        ...
        if (fromid == "" && toid == "" && start_time == "" && end_time==""){
            alert("请输入查询条件")
            return false;
        }
        window.location.href="${ctx}/admin/transfer/toWNCTTransferSearch?start_time="+start_time+"&end_time="+end_time+"&fromid="+fromid+"&toid="+toid;
    }

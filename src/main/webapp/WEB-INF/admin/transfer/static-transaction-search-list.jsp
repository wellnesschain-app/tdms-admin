...
<div class="x-body">
    <form class="layui-form x-center" action="${ctx}/admin/transfer/toFindStaticBonusSearch" method="get" style="width:800px">
        <div class="layui-form-pane" style="margin-top: 15px;">
            <div class="layui-form-item" style="width: 500px;margin: 0 auto;margin-bottom: 15px">
                <div class="layui-input-inline">
                    <input type="text" name="nickname"  placeholder="请输入用户ID" autocomplete="off" class="layui-input">
                </div>
    ...
    <xblock>共有数据：${staticBonus.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${staticBonus.list.size()==0}">
            <tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${staticBonus.list.size()>0}">
            <c:forEach items="${staticBonus.list}" var="t">
                <tr>
                    <td>
                    ...
                    <td >
                        <c:if test="${t.status==1}">
                            <span class="layui-btn layui-btn-warm layui-btn-mini">正常</span>
                        </c:if>
                    ...
                    <td>
                        <a style="cursor: pointer" onclick="showContent('${t.id}')">(点击查看明细)</a>
                    </td>
                </tr>
            </c:forEach>
        </c:if>
    ...
    <input type="hidden" id="pages" value="${staticBonus.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            $.ajax({
                url:"${ctx}/admin/transfer/getStaticBonus",
                type:"GET",
                ...
                success:function(res){
                    if(res.msg=="ok"){
                        ...
                        var tr = "";
                        var type="";
                        $("#pages").val(res.staticBonus.pages);
                        if(res.staticBonus.list.length<=0){
                            tr='<tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>'
                        }else{
                            $.each(res.staticBonus.list,function (i, v) {
                                if(v.status==1){
                                    statuss='<span class="layui-btn layui-btn-warm layui-btn-mini">正常</span>'
                                }else if(v.status==2){
                                    statuss='<span class="layui-btn layui-btn-danger layui-btn-mini">锁定</span>'
                                }
                            tr+='<tr><td><input type="checkbox" value="'+v.id+'" name=""></td>'+
                                '<td>' +v.id+'</td>' +
                                '<td >' +statuss+'</td>' +
                                '<td>' +v.count+'</td>' +
                                '<td><a style="cursor: pointer" onclick="showContent('+v.id+')">(点击查看明细)</a></td></tr>'
                            })
                        }
    ...
    function showContent(id) {
        layer.open({
            ...
            content:"${ctx}/admin/transfer/toFindStaticBonusRecord?id="+id
        });
    }

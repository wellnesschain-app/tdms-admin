...
<div class="x-body">
    <%--<form class="layui-form x-center" action="${ctx}/admin/member/search" method="get" style="width:800px">--%>
        ...
        <%--</div>--%>
    <%--</form>--%>
    <xblock>共有数据：${DynamicBonusRecord.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${DynamicBonusRecord.list.size()==0}">
            <tr><td colspan="8" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${DynamicBonusRecord.list.size()>0}">
            <c:forEach items="${DynamicBonusRecord.list}" var="d">
                <tr>
                    <td>
                    ...
                    <td>
                        <c:if test="${d.status==8}">
                            直推奖励
                        </c:if>
                        <c:if test="${d.status==9}">
                            发展奖励
                        </c:if>
                        <c:if test="${d.status==10}">
                            同级奖励
                        </c:if>
                        <c:if test="${d.status==11}">
                            级差奖励
                        </c:if>
    ...
    <input type="hidden" id="pages" value="${DynamicBonusRecord.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            var  id = '${id}';
            $.ajax({
                url:"${ctx}/admin/transfer/getFindDynamicRecord",
                type:"GET",
                ...
                success:function(res){
                    if(res.msg=="ok"){
                        ...
                        var tr = "";
                        var type="";
                        $("#pages").val(res.DynamicBonusRecord.pages);
                        if(res.DynamicBonusRecord.list.length<=0){
                            tr='<tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>'
                        }else{
                            $.each(res.DynamicBonusRecord.list,function (i, v) {
                                if(v.F04==1){

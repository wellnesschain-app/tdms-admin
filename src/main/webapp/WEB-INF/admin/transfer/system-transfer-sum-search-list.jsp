...
<div class="x-body">
    ...
    <xblock>共有数据：${SystemTransfersCount.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${SystemTransfersCount.list.size()==0}">
            <tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${SystemTransfersCount.list.size()>0}">
            <c:forEach items="${SystemTransfersCount.list}" var="t">
                <tr>
                    <td>
    ...
    <input type="hidden" id="pages" value="${SystemTransfersCount.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            var id = '${id}';
            $.ajax({
                url:"${ctx}/admin/transfer/getSystemTransferSearchCountList",
                type:"GET",
                data:{"pageNum":curr,"pageSize":10,"id":id,"account":account},
                dataType:"json",
                success:function(res){
                    if(res.msg=="ok"){
                        ...
                        var tr = "";
                        var type="";
                        $("#pages").val(res.SystemTransfersCount.pages);
                        if(res.SystemTransfersCount.list.length<=0){
                            tr='<tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>'
                        }else{
                            $.each(res.SystemTransfersCount.list,function (i, v) {

                            tr+='<tr><td><input type="checkbox" value="'+v.id+'" name=""></td>'+
                                '<td>' +v.id+'</td>' +
                                '<td>' +v.fbalance+'</td>' +
                                '<td >' +v.fusebalance+'</td>' +
                                '<td>' +v.fusdtbalance+'</td>' +
                                '<td>' +v.tbalance+'</td>' +
                                '<td >' +v.tusebalance+'</td>' +
                                '<td>' + v.fusdtbalance +'</td></tr>'
                            })
                        }
    ...
    function search() {
        ...
        window.location.href="${ctx}/admin/transfer/toSystemTransferCountSearchList?id="+id;
    }
</script>

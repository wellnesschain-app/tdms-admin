...
<div class="x-body">
    <%--<form class="layui-form x-center" action="${ctx}/admin/member/search" method="get" style="width:800px">
        ...
        <br>
    <xblock>共有数据：${documentary.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${documentary.list.size()==0}">
            <tr><td colspan="7" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${documentary.list.size()>0}">
            <c:forEach items="${documentary.list}" var="d">
                <tr>
                    <td>
                    ...
                    <td>
                        <a onclick="cancelOrder(${d.user_id},'${d.order_num}','${d.sale_address}','${d.value}')" style="cursor:pointer;">撤单</a>
                    </td>
                </tr>
            </c:forEach>
        </c:if>
    ...
    <input type="hidden" id="pages" value="${documentary.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            ...
            $.ajax({
                url:"${ctx}/admin/documentary/getDocumentarySearchList",
                type:"GET",
                data:{"pageNum":curr,"pageSize":10,"id":id,"start_time":start_time,"end_time":end_time},
                dataType:"json",
                    ...
                    if(res.msg=="ok"){
                        ...
                        if(res.documentary.list.length<=0){
                            ...
                            tr+='<tr><td><input type="checkbox" value="'+v.id+'" name=""></td>'+
                                ...
                                '<td >'+v.order_num+'</td>' +
                                '<td>' + statuss +'</td>' +
                                '<td><a onclick="cancelOrder(\''+v.user_id+'\',\''+v.order_num+'\',\''+v.sale_address+'\',\''+v.value+'\')" style="cursor:pointer;">撤单</a></td></tr>'
                            })
                        }
    ...
    function search() {
        ...
        window.location.href="${ctx}/admin/documentary/toDocumentarySearchList?start_time="+start_time+"&end_time="+end_time+"&id="+id;
    }

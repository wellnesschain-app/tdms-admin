...
<div class="x-nav">
            <span class="layui-breadcrumb">
              <a href="${ctx}/admin/toWelcome" href="javascript:;" ><cite style="cursor:pointer;">首页</cite></a>
              <a><cite>会员管理</cite></a>
              <a><cite>会员列表</cite></a>
            </span>
    ...
    <%--<form class="layui-form x-center" action="${ctx}/admin/user/userSearch" method="get" style="width:800px">
        <div class="layui-form-pane" style="margin-top: 15px;">
            <div class="layui-form-item" style="width: 500px;margin: 0 auto;margin-bottom: 15px">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="id"  placeholder="注册开始日期" autocomplete="off" class="layui-input">
                </div>
        ...
        <div class="demoTable">
            ...
            账号状态：
            <div class="layui-inline">
                <select name="lock" id="lock" style="height: 38px;">
                    ...
                    <td>
                        ...
                        <a title="编辑" href="javascript:;" onclick="wealth_edit('编辑','${ctx}/admin/user/toEditbalance?addr='+'${w.addr}'+'&id='+${w.id},'900','380')"
                           class="ml-5" style="text-decoration:none">
                            ...
                            <a onclick="wealth_edit('资产信息','${ctx}/admin/user/toEditbalance?addr='+'${w.addr}'+'&id='+${w.id},'750','500')" style="cursor:pointer;">查看</a>&nbsp|&nbsp<a onclick="locking(${w.id})" style="cursor:pointer;">锁定</a>&nbsp|&nbsp<a onclick="unlock(${w.id})" style="cursor:pointer;">解锁</a>
...
<script>
                        ...
                        if(res.wallets.list.length<=0){
                            ...
                            $.each(res.wallets.list,function (i, v) {
                               ...
                               /* '<td>' +v.name+'</td>' +*/
                                ...
                                '<td><a onclick="showContent('+v.id+',\''+v.addr+'\')" style="cursor:pointer;">查看</a>&nbsp|&nbsp<a onclick=" locking('+v.id+')" style="cursor:pointer;">锁定</a>&nbsp|&nbsp<a onclick="unlock('+v.id+')" style="cursor:pointer;">解锁</a></td></tr>'
    ...
    function showContent(id,addr) {
        layer.open({
            ...
            content:"${ctx}/admin/user/toEditbalance?id="+id + "&addr="+addr
     ...
     */
    function unlock(id){
        layer.confirm('确认解锁ID为'+id+'的用户吗？',function(){
            $.post("${ctx}/admin/user/userUnlock",{id:id},function(res){
                if(res.msg=='ok'){
                    layer.alert("成功！",{icon:"6"},function () {
                        location.reload();
                    })
    ...
    function search() {
        ...
        if (id == "" && account == "" && start_time == "" && end_time=="" && lock ==""){
            alert("请输入查询条件")
            return false;
        }
        window.location.href="${ctx}/admin/user/userSearch?start_time="+start_time+"&end_time="+end_time+"&id="+id+"&account="+account+"&lock="+lock;
    }

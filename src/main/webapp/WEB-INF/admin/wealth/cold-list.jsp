...
<div class="x-body">
    ...
    <xblock>共有数据：${colds.total} 条</span></xblock>
        ...
        <c:if test="${colds.list.size()>0}">
            <c:forEach items="${colds.list}" var="c">
                <tr>
                    ...
                    <td>
                            <fmt:formatDate value="${c.buyinTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/>
                    </td>
                    <td>
                            <fmt:formatDate value="${c.lastTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/>
                    </td>
                    <td>
                            ${c.releases}
                    </td>
                    ...
                    if(res.msg=="ok"){
                        ...
                        if(res.colds.list.length<=0){
                            ...
                            $.each(res.colds.list,function (i, v) {
                                if(v.colded==null||v.colded==''){
                                    colded="暂无";
                                }
                                if(v.ternouttime==null||v.ternouttime==''){
                                    timeout="无"
                                }
                                if(v.ternouttime!=null){
                                    timeout=timeFormat(v.ternouttime);
                                }
                                if(v.stat==0){
                                    status='<span class="layui-btn layui-btn-success layui-btn-mini">未解冻</span>';
                                }
                                if(v.stat==1){
                                    status='<span class="layui-btn layui-btn-warm layui-btn-mini">已解冻</span>';
                                }
                            tr+='<tr><td><input type="checkbox" value="'+v.id+'" name=""></td>'+
                                '<td><u style="cursor:pointer">' +v.uid +'</u></td><td>'+
                                v.wid+'</td><td >' +
                                v.val+'</td><td>' +
                                v.rate+'</td><td>' +
                                timeFormat(v.buyinTime)+'</td><td>' +
                                timeFormat(v.lastTime)+'</td><td>' +
                                v.releases +'</td><td>'+
                                v.surplus +'</td><td>'+
                                status +
                                '</td></tr>'

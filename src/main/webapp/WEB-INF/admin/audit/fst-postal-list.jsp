...
<div class="x-body">
    ...
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${postalTransfers.list.size()==0}">
            <tr><td colspan="4" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${postalTransfers.list.size()>0}">
            <c:forEach items="${postalTransfers.list}" var="p">
                <tr>
                    <td>
                        ${p.id}
                    </td>
                    ...
                    <td>
                            <fmt:formatDate value="${p.time}" pattern="yyyy年MM月dd日 HH:mm:ss"/>
                    </td>
                </tr>
            </c:forEach>
        </c:if>
            ...
            $.ajax({
                ...
                success:function(res){
                    if(res.msg=="ok"){
                        ...
                        var tr = "";
                        var authen="";
                        $("#pages").val(res.postalTransfers.pages);
                        if(res.postalTransfers.list.length<=0){
                            tr='<tr><td colspan="4" style="text-align: center;color: #ccc">暂无数据</td></tr>'
                        }else{
                            $.each(res.postalTransfers.list,function (i, v) {
                            tr+='<tr><td>'+v.id+'</td>'+
                                '<td><u style="cursor:pointer">'+v.pid+'</u></td>' +
                                '<td><u style="cursor:pointer">' +v.txHash+'</u></td>' +

...
<div class="x-body">
        ...
        <div class="layui-form-pane" style="margin-top: 15px;">
            <div class="layui-form-item" style="width: 500px;margin: 0 auto;margin-bottom: 15px">
                <div class="layui-input-inline">
                    <input type="text" name="nickname"  placeholder="请输入ID" autocomplete="off" class="layui-input">
                </div>
    ...
    <xblock>共有数据：${unAdoptSearch.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${empty unAdoptSearch}">
            <tr><td colspan="9" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${not empty unAdoptSearch}">
                <tr>
                    <td>
                        ${unAdoptSearch.id}
                    </td>
                    ...
                    <td>
                        <c:if test="${unAdoptSearch.isAuthen==0}">
                            <span class="layui-btn layui-btn-warm layui-btn-mini">
                                未实名
                            </span>
                        </c:if>
                        <c:if test="${unAdoptSearch.isAuthen==1}">
                            <span class="layui-btn layui-btn-succes layui-btn-mini">
                                已实名
                            </span>
                            <a style="cursor: pointer" onclick="authens('认证信息','${ctx}/admin/user/authenDetails?addr=${unAdoptSearch.from}',600,380)">(点击查看)</a>
                        </c:if>
                    </td>
                    <td >
                        ${unAdoptSearch.fee}
                    ...
                    <td>
                        ${unAdoptSearch.actualValue}
                    </td>
                    <td>
                        <a style="cursor: pointer" onclick="textDetails('${unAdoptSearch.text}')">(点击查看)</a>
                    </td>
                    <td>
                         <fmt:formatDate value="${unAdoptSearch.applyTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/>
                    </td>
                    <td>
                        <fmt:formatDate value="${unAdoptSearch.auditTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/>
                    </td>
                </tr>
        </c:if>
        </tbody>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            $.ajax({
                url:"${ctx}/admin/audit/getFSTunAdoptList",
                type:"GET",

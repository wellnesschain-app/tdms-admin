...
<div class="x-body">
    ...
    <xblock>共有数据：${adoptSearch.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <c:if test="${not empty adoptSearch}">
                <tr>
                    ...
                    <td>
                        <c:if test="${adoptSearch.isAuthen==0}">
                            <span class="layui-btn layui-btn-warm layui-btn-mini">
                                未实名
                            </span>
                        </c:if>
                        <c:if test="${adoptSearch.isAuthen==1}">
                            <span class="layui-btn layui-btn-succes layui-btn-mini">
                                已实名
                            </span>
                            <a style="cursor: pointer" onclick="authens('认证信息','${ctx}/admin/user/authenDetails?addr=${adoptSearch.from}',600,380)">(点击查看)</a>
                        </c:if>
                    </td>
                    <td >
                        ${adoptSearch.fee}
                    ...
                    <td>
                        ${adoptSearch.actualValue}
                    </td>
                    <td>
                         <fmt:formatDate value="${adoptSearch.applyTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/>
                    </td>
                    <td>
                        <fmt:formatDate value="${adoptSearch.auditTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/>
                    </td>
                </tr>
        </c:if>
        </tbody>
    </table>
    <input type="hidden" id="pages" value="${adoptSearch.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            $.ajax({
                url:"${ctx}/admin/audit/getFSTAdoptList",
                type:"GET",

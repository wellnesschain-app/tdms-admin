...
<div class="x-body">
    ...
    <xblock>共有数据：${auditSearch.total} 条</span></xblock>
    <table class="layui-table">
        ...
        <c:if test="${not empty auditSearch}">
                <tr>
                    ...
                    <td>
                        <c:if test="${auditSearch.isAuthen==0}">
                            <span class="layui-btn layui-btn-warm layui-btn-mini">
                                未实名
                            </span>
                        </c:if>
                        <c:if test="${auditSearch.isAuthen==1}">
                            <span class="layui-btn layui-btn-succes layui-btn-mini">
                                已实名
                            </span>
                            <a style="cursor: pointer" onclick="authens('认证信息','${ctx}/admin/user/authenDetails?addr=${auditSearch.from}',600,380)">(点击查看)</a>
                        </c:if>
                    </td>
                    <td >
                        ${auditSearch.fee}
                    ...
                    <td>
                        ${auditSearch.actualValue}
                    </td>
                    <td>
                            <fmt:formatDate value="${auditSearch.applyTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/>
                    </td>
                    <td>
                        <a onclick="adopt(${auditSearch.id})" style="cursor:pointer;">通过</a>&nbsp|&nbsp<a onclick="reject(${auditSearch.id},'${auditSearch.from}')" style="cursor:pointer;">驳回</a>
                    </td>
                </tr>
        </c:if>
        </tbody>
    </table>
    <input type="hidden" id="pages" value="${auditSearch.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            $.ajax({
                url:"${ctx}/admin/audit/getFSTAuditList",
                type:"GET",
                    ...
                    if(res.msg=="ok"){
                        ...
                        if(res.audits.list.length<=0){
                            ...
                            tr+='<tr><td>'+v.id+'</td>'+
                                ...
                                '<td><a onclick="adopt('+v.id+')" style="cursor:pointer;">通过</a>&nbsp|&nbsp<a onclick="reject('+v.id+',\''+v.from+'\')" style="cursor:pointer;">驳回</a></td></tr>'
                            })
                        }

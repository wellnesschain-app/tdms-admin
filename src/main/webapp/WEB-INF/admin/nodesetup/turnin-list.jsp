...
<div class="x-body">
    ...
    <table class="layui-table">
        ...
        <tbody id="tbody">
        <c:if test="${turnins.list.size()==0}">
            <tr><td colspan="6" style="text-align: center;color: #ccc">暂无数据</td></tr>
        </c:if>
        <c:if test="${turnins.list.size()>0}">
            <c:forEach items="${turnins.list}" var="t">
                <tr>
                    <td>
    ...
    <input type="hidden" id="pages" value="${turnins.pages}">
    <div id="page"></div>
</div>
...
<script>
    layui.use(['laydate','element','laypage','layer'], function(){
        ...
        function getMemberList(curr) {
            $.ajax({
                url:"${ctx}/admin/user/getUserGoldEntryList",
                type:"GET",
                ...
                success:function(res){
                    if(res.msg=="ok"){
                        ...
                        $("#pages").val(res.turnins.pages);
                        if(res.turnins.list.length<=0){
                            tr='<tr><td colspan="6" style="text-align: center;color: #ccc">暂无数据</td></tr>'
                        }else{
                            $.each(res.turnins.list,function (i, v) {
